...
public class GUI_automation_GeneralMeths {
	public static WebDriver _driver;
	private static String URL = null;
	private static String fileSeperator = System.getProperty("file.separator");

	public GUI_automation_GeneralMeths(WebDriver driver){
		_driver = driver;
	...
	public void waitForVisibility(final By theElement,int timeoutInSeconds) {
		if(!_driver.findElement(theElement).isDisplayed()){
	...
	public void waitForAnElementToBeVisible(WebElement element,int timeoutInSeconds) {
		try {
			WebDriverWait wait = new WebDriverWait(_driver, timeoutInSeconds);
			wait.until(ExpectedConditions.visibilityOf(element));
		} catch (Exception e) {
			Reporter.log("There was an issue in finding the webelement, " + element.getClass() + e.getMessage());
			e.printStackTrace();
		}
	}

	public void waitForAnElementToBeClickable(WebElement element,int timeoutInSeconds) {
		try {
			WebDriverWait wait = new WebDriverWait(_driver, timeoutInSeconds);
			wait.until(ExpectedConditions.visibilityOf(element));
		} catch (Exception e) {
